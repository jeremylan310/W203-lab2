...
> 1. **IID Data:** This condition is rarely met perfectly and this is not the exception. Videos from the same author or YouTube channel, or in the same language are most likely not indepdendent from one other. With that said, given the randomness of the sample process from the authors, we can assume this condition to be met. 
> 2. **No Perfect Colinearity:** We can see that R hasn't dropped any of our variables in the model or failed to run the regression. This means that the $X$ matrix is invertible, thus no perfect colinearity exists and the BLP exists.
