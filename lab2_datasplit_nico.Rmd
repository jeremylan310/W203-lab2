...
ba_plot3 = fb_data_exp %>% ggplot() + 
  aes(x=log(page_ttl_likes)) + 
  geom_histogram(bins=20, color="blue", fill="blue", alpha=0.5) +
  labs(
    x="Log of Brand Awareness"
  )

ba_plot4 = fb_data_exp %>% ggplot() + 
  aes(x=log(page_ttl_likes)) + 
  geom_boxplot(color="blue", fill="blue", alpha=0.5) +
  labs(
    x="Log of Brand Awareness"
...
grid.arrange(ba_plot1, ba_plot2, ba_plot3, ba_plot4,
             ncol=2, nrow=2,
             top="Distribution and BoxPlot of Total Likes on FB page (Brand Awareness)")
```
...
\begin{align*}
\widehat{\log(engaged\_users)}=\beta_0 &+ \beta_1 \text{ } type + \beta_2 \text{ } category\\ 
&+ \beta_3 \text{ } paid + \beta_4 \text{ } \log(brand\_awareness) + \beta_5 \text{ } day\_of\_week + \beta_6 \text{ } period\_of\_day
\end{align*}
...
\begin{align*}
\widehat{\log(engaged\_users)}=\beta_0 &+ \beta_1 \text{ } type + \beta_2 \text{ } category + \beta_3 \text{ } paid + \beta_4 \text{ } \log(brand\_awareness) \\
& + \beta_5 \text{ } day\_of\_week + \beta_6 \text{ } period\_of\_day + \beta_7 \text{ } type*category
\end{align*}
...
All the different types of posts are significant to an $\alpha$ level of 0.05 across all models, with Link being the omitted type. From the magnitude of the coefficients we can see that Video, Status and Photo perform much better than Link, with Status yielding the highest increase on engaged users. All these coefficients are quite stable across models as well. As for the categories, none of them are significant at a 0.05 $\alpha$ level. We also see that the Product coefficient alternates sign in different models, implying the possibility of high correlation among different variables.
